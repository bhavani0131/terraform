...
# Works perfectly whether enable_read_replica is true or false!
output "rds_read_replica_endpoint" {
  description = "Read replica endpoint for read-only queries. Null when enable_read_replica = false."
  value       = var.enable_read_replica ? aws_db_instance.read_replica[0].endpoint : null
}

# FIXED: Commented out because manage_master_user_password was removed in rds.tf
